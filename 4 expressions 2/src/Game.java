...
public class Game {
	...
	public void populateArena() {
		...
		/*for (int i = 0; i < heals.length; i++) {
			heals[i] = new Heal((int) (Math.random()*arena.xSize), (int) (Math.random()*arena.ySize));
			while (heals[i].get_x() == slayer.get_x_pos() && heals[i].get_y() == slayer.get_y_pos()) {
				heals[i] = new Heal(((int) (Math.random()*arena.xSize), (int) (Math.random()*arena.ySize)); //check if the heal is in the vampire position as well
			}
		}*/
	...
	public void playGame() {
		...
		while(true) {
			...
			vampLocation += getNumVampsAt(x, y - 1);
			slayer.setNumLives(slayer.getNumLives() - vampLocation); //add numlives when heal is picked up
